...
class ToolRegistry(
    ...
    suspend fun execute(name: String, argumentsJson: String, conversationId: String = "default"): ToolResult {
        val tool = tools[name]
            ?: run {
                com.amayra.maya.core.AgentDebugBus.toolRejected(name, argumentsJson, "Unknown tool: $name")
        ...
        val args: JsonObject = try {
            ...
            com.amayra.maya.core.AgentDebugBus.toolRejected(name, argumentsJson, "Malformed tool arguments: ${t.message}")
        ...
        if (tool.risk == RiskLevel.CONFIRM || tool.risk == RiskLevel.DANGEROUS) {
            ...
            val eventId = com.amayra.maya.core.AgentDebugBus.toolStarted(tool.name, argumentsJson)
            val ok = confirmationFlow("Maya wants to run: ${tool.name}", details)
            if (!ok) {
                MayaLog.i("TOOLS", "User declined tool ${tool.name}")
                com.amayra.maya.core.AgentDebugBus.toolFinished(eventId, "declined", "User declined", 0)
                return ToolResult.Failure("User declined to run ${tool.name}.")
            }
            // Confirmed: keep the running event so the panel shows the full arc.
            return executeTracked(tool, args, argumentsJson, conversationId, preStartedId = eventId)
        }

        return executeTracked(tool, args, argumentsJson, conversationId)
    }

    /** Real execution with live debug-bus tracking (duration, outcome, redacted summary). */
    private suspend fun executeTracked(
        tool: Tool,
        args: JsonObject,
        argumentsJson: String,
        conversationId: String,
        preStartedId: Long? = null,
    ): ToolResult {
        val eventId = preStartedId ?: com.amayra.maya.core.AgentDebugBus.toolStarted(tool.name, argumentsJson)
        ...
        return try {
            ...
            com.amayra.maya.core.AgentDebugBus.toolFinished(
                eventId,
                when (result) {
                    is ToolResult.Success -> "success"
                    is ToolResult.Failure -> "failure"
                    is ToolResult.ConfirmRequired -> "confirm"
                },
                summary,
            ...
            com.amayra.maya.core.AgentDebugBus.toolFinished(
                eventId, "failure", "Tool ${tool.name} failed: ${t.message}",
                (System.nanoTime() - startedAtNanos) / 1_000_000,
            )
