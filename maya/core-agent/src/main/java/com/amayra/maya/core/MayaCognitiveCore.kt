...
class MayaCognitiveCore(
    ...
    val ai: AiClient,
    val voice: AssistantVoice,
    val avatar: com.amayra.maya.feature.AvatarPort? = null
) {
    ...
    /** Wire the WhatsApp auto-reply engine (registered after DI wiring). */
    private val autoReplyEngine: com.amayra.maya.feature.LateInitExecutable?
        get() = AssistantFeatureHolder.postWiredExecutable
        ...
        scope.launch {
            ...
            if (last != null) {
                ...
                turns.value = db.messageDao.forConversation(conversationId).map {
                    // Persisted error turns carry their kind in toolName ("error:<kind>").
                    val errKind = it.toolName
                        ?.takeIf { tn -> tn.startsWith("error:") }
                        ?.removePrefix("error:")
                    ChatMessage(
                        role = it.role,
                        content = it.content,
                        ...
                        toolName = if (errKind != null) "error" else it.toolName,
    ...
    private suspend fun handleNotification(ev: MayaaEvent) {
        ...
        if (ev.packageName?.startsWith("com.whatsapp") == true) {
            val engine = autoReplyEngine ?: return
            if (engine.isInitialized()) {
                val contact = ev.title ?: return
                val text = ev.text ?: return
                val reply = engine.execute(appContext, contact, text)
                if (reply != null) {
    ...
    /**
     * Re-send the most recent user message (chat-UI Retry action).
     * Returns false when there is nothing to retry; runs in [scope].
     */
    /** User-requested cancellation of the in-flight turn (agent loop stops
     *  at the next boundary; already-started tool execution finishes). */
    ...
    fun retryLastUser(): Boolean {
        val lastUser = turns.value.lastOrNull { it.role == "user" && it.errorKind == null } ?: return false
        scope.launch { onUserMessage(lastUser.content, lastUser.imageUri) }
        return true
    }
    ...
    suspend fun onUserMessage(text: String, imageUri: String? = null) {
        ...
        AgentDebugBus.turnStarted(text)
        ...
        OpenAppIntent.match(text)?.let { m ->
            ...
            AgentDebugBus.turnFinished(spoken)
            return
        }
        StateBus.setState(AssistantState.Processing)
        ...
        val persona = AssistantFeatureHolder.personaPort?.active?.value
        val personaBlock = persona?.let { "\n\n[Active persona: ${it.displayName}. ${it.systemPrompt}]" } ?: ""
        ...
        try {
            ...
            var toolRounds = 0
            while (needsAnotherRound && guard < MAX_TOOL_ROUNDS) {
                guard++
                needsAnotherRound = false
                if (cancelRequested) {
                    MayaLog.i("CORE", "Turn cancelled by user after $toolRounds tool round(s)")
                    finalText = "Rok diya."
                ...
                val msgs = buildList {
                    ...
                    // Error turns are UI diagnostics, not conversation history —
                    // replaying them to the provider wastes context and can
                    // confuse tool-call round-trips.
                    addAll(turns.value.takeLast(20).filter { it.errorKind == null })
                }
                ...
                if (error != null) {
                    ...
                    // Persist a structured error turn so the chat UI can render a
                    // prominent, actionable banner (retry / fix-in-settings).
                    val kind = when (error) {
                        is AiError.RateLimit -> "rate_limit"
                        is AiError.Authentication -> "auth"
                        is AiError.Network -> "network"
                        is AiError.Timeout -> "timeout"
                        is AiError.Unsupported -> "unsupported"
                        is AiError.Provider -> "provider"
                    }
                    ...
                    turns.value += ChatMessage(
                        role = "assistant", content = "⚠️ $errorDetail",
                        toolName = "error", errorKind = kind
                    )
                    AgentDebugBus.turnError("$kind: $errorDetail")
                    return
                }
            ...
            AgentDebugBus.turnFinished(if (toolRounds > 0) "$finalText ($toolRounds tool round(s))" else finalText)
            // Use THIS turn's flow-read prefs (`p`), never prefsCache: the cache is
            // null until the app-scope collector runs, and a null here silently
            // dropped every spoken reply (observed: chat fine, voice dead).
            if (p.autoSpeak) {
            ...
            AgentDebugBus.turnError("exception: ${t.message}")
        }
    }
    ...
    companion object {
        ...
        /** Max AI→tool→AI rounds per turn (configurable agent-iteration cap). */
