...
import com.amayra.maya.accessibility.GuardianEnrollTool
import com.amayra.maya.accessibility.GuardianStatusTool
...
import com.amayra.maya.accessibility.WakeWordToggleTool
...
object AppGraph {
    ...
    fun init(context: Context) {
        ...
        voice = VoiceController(app, onFinalTranscript = { transcript ->
            ...
            v.personaVoiceProvider = { personas.resolvedVoice(settings.prefsCache?.ttsModel) }
            ...
            scope.launch {
                settings.prefs.collect { v.enginePreference = it.ttsEngine }
            }
        }
        ...
        // -- cognitive core ---------------------------------------------------
        core = MayaCognitiveCore(app, settings, db, secure, eventBus, registry, ai, voice, avatar)
        voice.bindAvatar(
        ...
        core.autoReplyEngine = WhatsAppAutoReplyEngine(settings, ai)
    ...
    private fun startBackground() {
        ...
        scope.launch {
            settings.prefs.collect { voice.enginePreference = it.ttsEngine }
        }
    }
    ...
    private suspend fun confirmViaUi(title: String, details: String): Boolean =
        kotlinx.coroutines.suspendCancellableCoroutine { cont ->
            core.pendingConfirm.value = MayaCognitiveCore.PendingConfirm(
